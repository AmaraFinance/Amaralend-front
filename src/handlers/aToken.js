import ATokenContract from '@/contracts/AErc20Delegator.json';
 ...
 * Market handler
 */
export default class ATokenDelegator {
  /**
  ...
  constructor(address = '') {
    ...
    this.instance = new web3.eth.Contract(ATokenContract.abi, address);
    ...
    this.instanceToken = this.instance.methods.underlying().call().then((tokenAddress) => new Token(tokenAddress))
      .catch((err) => {
        console.log(err)
      });
  ...
  // get getCollateralFactorMantissa(){
  // 	return new Promise((resolve, reject) => {
  // 		this.instance.methods.collateralFactorMantissa()
  // 		.then((collateralFactorMantissa) => Number(collateralFactorMantissa))
  // 			.then(resolve)
  // 			.catch(reject);
  // 	});
        ...
        .then((allowance) => {
          ...
          if (allowance < amount) {
            return _token.approve(this.instanceAddress,
              "100000000000000000000000000000000", from)
          }
        }).then(() =>
          send(this.instance.methods.mint(amount), from)
        )
        .then(resolve)
        .catch((err) => {
          console.log(err);
      ...
      this.instanceToken
        ...
        .then((allowance) => {
         allowance = Number(allowance);
          if (allowance < amount) {
            return _token.approve(this.instanceAddress,
              "115792089237316195423570985008687907853269984665640564039457584007913129639935", from)
          }
        })
        .then(() => send(this.instance.methods.repayBorrow(amount), from))
        .then(resolve)
        .catch((err) => {
          console.log(err);
          reject(err);
